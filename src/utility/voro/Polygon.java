...
import static utility.voro.Global.slope_tolerance;
...
public class Polygon {
    ...
    void add_point(Point $p) {
        for (int i = 0; i < npoints; i++) {
            if ($p.get_distance_to(points[i].x, points[i].y, points[i].z) < distance_tolerance) {
                return;
            }
        }
        points[npoints] = $p;
    ...
    void delete_point(Point $p) {
        // this is not an efficient way to remove points, just the easiest:
        //     for reducing lots of large polygons, this should target only the segment to be removed,
        //     rather than rebuilding the entire polygon
        int ntemp = npoints;
    ...
    public void render2D(PApplet surface) {
        surface.beginShape();
        for (int i = 0; i < npoints; i++) {
            surface.vertex(points[i].x, points[i].y);
        }
    ...
    */
    ...
    public void render(PApplet surface) {
        surface.beginShape();
        for (int i = 0; i < npoints; i++) {
            surface.vertex(points[i].x, points[i].y, points[i].z);
        }
        surface.endShape(CLOSE);
    /*
    for(int i=0;i<npoints;i++){
